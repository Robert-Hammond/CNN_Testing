...
class HilbertFlatten(Layer):
    """Flattens the input using a hilbert curve. Does not affect the batch size.
    ...
    def __init__(self, data_format=None, **kwargs):
        super(HilbertFlatten, self).__init__(**kwargs)
        ...
        if context.executing_eagerly():
            ...
            if rank == 1:
                ...
                if non_batch_dims.is_fully_defined():
                    last_dim = int(functools.reduce(
                        operator.mul, non_batch_dims))
    ...
    def get_config(self):
        config = super(HilbertFlatten, self).get_config()
